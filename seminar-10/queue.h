 ...
 * \note If the queue creation fails, the \c qptr parameter will
 *       remain unchanged.
 *
 * \param[out]  qptr         queue to initialize
 ...
 */
bool queue_create(struct queue **qptr, size_t element_size, size_t capacity);

/**
 * \brief Deallocates the queue.
 *
 * This function will free all memory associated with the \c queue, including
 * stored data. Behaviour of further operations on the @c queue parameter,
...
/**
 * \brief Test the queue emptiness.
 *
